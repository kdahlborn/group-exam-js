...
function validateForm() {
	...
	try {
		if (name < 5 || name > 10) {
			throw {
				msg: 'Name must contain 5-10 letters',
				nodeRef: globalRef.trainerName,
			};
		} else if (age < 10 || age > 15 || isNaN(age)) {
			throw {
				msg: 'You must be between 10-15 year old',
				nodeRef: globalRef.trainerAge,
			};
		} else if (!boy.checked && !girl.checked) {
			throw {
				msg: 'Choose if you are a boy or girl',
				nodeRef: document.querySelector('.radioWrapper'),
			};
		}
		...
		document.querySelector('#nick').classList.remove('error');
		document.querySelector('#age').classList.remove('error');
		error.nodeRef.classList.add('error');
		error.nodeRef.focus();
		return false;
	}
}
...
function displayHighScores(sortedScore) {
	...
	for (let i = 0; i < list.length; i++) {
		...
		textRef.textContent = `${list[i].name}, ${list[i].age}, ${list[i].gender}, ${list[i].time} ms`;
